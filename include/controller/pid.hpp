...
template <isQuantity In, isQuantity Out>
class PID : public Controller<In, In, Out> {
  ...
  virtual Out update(const In &input) override { // todo: test this all
    ...
    if ((threshold != 0) && (max != 0) && (error > threshold)) {
      ...
      if ((error < 0) != (m_prev_error < 0))
        m_integral *=
            m_config.integralCut; // cut down I if the sign of the error changes
      else if ((m_config.integralMax != In(0.0)) &&
               (m_integral >= m_config.integralMax.convert(m_config.inBase)))
        m_integral =
            std::copysign(m_config.integralMax.convert(m_config.inBase),
                          m_integral); // cap I
      else
        m_integral += error;
